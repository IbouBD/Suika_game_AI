...
for i in range(500):
    ...
    # Initialisation du jeu (une seule fois par épisode)
    pygame.init()
    screen = pygame.display.set_mode((WIDTH, HEIGHT))

    # Initialisation de l'état initial du jeu (vous devrez peut-être ajuster cela)
    ...
    #print("shape :",handler.data["mapper"])
    ...
    def compute_state_vector(particles, next_particle):
        ...
        # Ajoutez ici le calcul de l'état en fonction du numéro de pas de temps
        # Par exemple, vous pouvez utiliser time_step pour ajuster certains aspects de l'état en fonction du temps.
    ...
    state_vector = compute_state_vector(particles, next_particle)
    #state_f = np.ravel(state_vector)
    #normalized_state = state_f / max(state_f)
    state = state_vector
    ...
    def reward(particles, score, terminated, fusion, game_over, prev_score, PAD, state_vector):

        reward = score
        # Récompense pour chaque fusion
        reward += 20 * fusion
        
        # Récompense en fonction de la position
        for i in range(1, 5):
            ...
            for adjacent_particle in particles:
                reward += distance_to_edge / PAD[1] * 1
    ...
    while terminated == False:
        ...
        current_state = compute_state_vector(particles, next_particle)
        ...
        next_state = compute_state_vector(particles, next_particle)
        ...
        rewards = reward(score = score, terminated = terminated, fusion = handler.data["fusion"], prev_score=prev_score, game_over= game_over,  PAD=PAD, state_vector=state_vector, particles=particles)
